...
return Column(children: [
  Text(DateFormat('LLL').format(date), style: TextStyle(fontSize: 30.0, color: Colors.black),),
  DayView(
    date: date,
  ...
  List<FlutterWeekViewEvent> convertEvents(List<Event> events) {
    List<FlutterWeekViewEvent> newEvents = List.empty(growable: true);

    for(int i=0; i<events.length; ++i) {
      newEvents.add(FlutterWeekViewEvent(title: events[i].name, description: "", start: events[i].range.start, end: events[i].range.end));
    }
